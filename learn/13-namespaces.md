...
Open `users/phil-personal.nix` and `modules/home/zsh.nix` alongside this.

## Step 1: someone sets a phix option

From `users/phil-personal.nix`:
...
2. Checks that `phix.zsh.aliases` is `attrsOf str` — it is
...
- No type checking on your grouping of options
