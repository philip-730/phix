...
#
# These are the main ways Nix lets you name things, avoid repetition, and
# build up values from other values. You'll see all of these in phix.
#
# Run: nix eval --file learn/05-let-inherit.nix
#
# ──────────────────────────────────────────────────────────────────────────────

{
  ...
  # let bindings can reference each other (in order)
  chained = let
    base = 10;
    doubled = base * 2;
  ...
  # → "--delete-older-than +5"
  #
  # This exact pattern is in: modules/common/nix.nix:4+53
  ...
  withExample =
    with { a = 1; b = 2; c = 3; };
    a + b + c;  # → 6  (a, b, c are in scope from the attrset)

  # Without `with`:  { a = 1; b = 2; c = 3; }.a + { a = 1; b = 2; c = 3; }.b ...
  ...
  # lib/default.nix uses `rec` so mkNixosHost and mkDarwinHost can call each
  # other if needed:
  #   rec {
  #     mkNixosHost = ...;
  #     mkDarwinHost = ...;
  #   }
  # See: lib/default.nix:7

  # ── toString ─────────────────────────────────────────────────────────────────
  #
  # Nix strings don't auto-coerce numbers. You need toString to embed them.
  # This is why modules/common/nix.nix:53 uses:
  #   options = "--delete-older-than +${toString cfg.gc.keepGenerations}";
  #
