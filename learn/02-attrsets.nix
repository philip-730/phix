...
# pairs. Almost everything in a Nix config is an attrset nested inside other
# attrsets.
#
# When you write:
#   nix.settings.substituters = [ "https://cache.nixos.org" ];
#
# That's not special syntax — it's just a shorthand for a deeply nested attrset:
#   { nix = { settings = { substituters = [...]; }; }; }
#
# Run: nix eval --file learn/02-attrsets.nix
#
# ──────────────────────────────────────────────────────────────────────────────

{
  ...
  # In practice you'd use let to avoid repeating yourself:
  ...
  ageOrDefault = { name = "Philip"; }.age or 0;  # → 0  (key missing, use default)
  ...
  #
  # For deep merging, Nix has lib.recursiveUpdate (covered in 12-mkhelpers.nix).
  # See it used in: modules/home/git.nix:78 — lib.recursiveUpdate for extraConfig.
  ...
  # This is used in phix when generating configs keyed by username, etc.
